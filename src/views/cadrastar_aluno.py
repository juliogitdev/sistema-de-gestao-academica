...
def exibir_tela_cadastrar_aluno(root):
    ...
    # Frame esquerdo
    frame_esquerdo = ctk.CTkFrame(root, fg_color=interface.COR_VERDE)
    ...
    frame_texto_boas_vindas = ctk.CTkFrame(frame_esquerdo, fg_color=interface.COR_VERDE, corner_radius=0)
    ...
    # Frame direito
    frame_direito = ctk.CTkFrame(root, fg_color=interface.COR_BACKGROUND)
    ...
    ctk.CTkLabel(frame_direito, text="CRIAR PERFIL", font=('League Spartan', 40, 'bold'),
                 anchor="center", text_color=interface.COR_VERDE).grid(column=1, row=1, sticky='s')

    #Crinando o frame onde irá ficar as entradas. (Matricula, nome)
    frame_input = ctk.CTkFrame(frame_direito, fg_color=interface.COR_BACKGROUND)
    ...
    input_matricula = ctk.CTkEntry(frame_input, fg_color=interface.COR_CINZA, placeholder_text='Matrícula', width=250, height=50,
                 corner_radius=15, placeholder_text_color=interface.COR_BACKGROUND, font=('League Spartan', 20),
                 border_width=0)
    input_matricula.grid(column=0, row=0, sticky='s')

    input_nome = ctk.CTkEntry(frame_input, fg_color=interface.COR_CINZA, placeholder_text='Nome', width=250, height=50,
                 corner_radius=15, placeholder_text_color=interface.COR_BACKGROUND, font=('League Spartan', 20),
                 border_width=0)
